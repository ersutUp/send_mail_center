...
class RequestUtilTest {
    ...
    @BeforeAll
    static void requestUtil(){
        requestUtil = new RequestUtil("http://127.0.0.1:8080","Smf7g60uDvpzwSPHGE2v");
    }

    @Test
    @Order(1)
    void sendSimpleMailV1() {
        Result<SendMailResult> result = requestUtil.sendSimpleMailV1(SendMailParam
                .builder()
                .title("sdk测试")
    ...
    @Order(2)
    void getMailByIdV1() {
        Result<GetMailByIdResult> result = requestUtil.getMailByIdV1(mailRecordId == null ? 43L : mailRecordId);
        System.out.println(result.toString());
        Assertions.assertEquals(result.getCode(), SystemResultCode.SUCCESS.getCode());
